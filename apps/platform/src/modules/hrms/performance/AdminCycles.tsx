...
export function AdminCycles() {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">Review cycles</h2><p className="mt-1 text-sm text-text-secondary">Create a period, assign reviewers, and track completion.</p></div>{canWrite && <HrButton onClick={() => setCreating(true)}><Plus size={16} /> Create cycle</HrButton>}</div>
    {cycles.isError ? <PerformanceError error={cycles.error} retry={() => cycles.refetch()} /> : <TableCard><DataTable<ReviewCycle> data={cycles.data ?? []} keyField="id" loading={cycles.isLoading} emptyMessage="No review cycles yet. Create a cycle to begin." columns={[
      ...
      { key: 'period', header: 'Review period', render: cycle => <span className="whitespace-nowrap">{cycle.periodStart || 'Not set'} &rarr; {cycle.periodEnd || 'Not set'}</span> },
      { key: 'status', header: 'Status', render: cycle => <HrStatusPill tone={cycle.status === 'ACTIVE' ? 'ok' : cycle.status === 'CLOSED' ? 'info' : 'gray'}>{cycle.status.toLowerCase()}</HrStatusPill> },
...
function CycleDrawer({ cycle, canInitiate, onClose }: { cycle: ReviewCycle; canInitiate: boolean; onClose: () => void }) {
  ...
  return <HrDrawer title={cycle.name} width="max-w-2xl" onClose={() => { if (!busy) onClose() }}>
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3"><div><p className="text-sm text-text-secondary">{cycle.periodStart} &rarr; {cycle.periodEnd}</p><p className="mt-1 text-xs text-text-secondary">Appraisal review cycle</p></div><HrStatusPill tone={status === 'ACTIVE' ? 'ok' : status === 'CLOSED' ? 'info' : 'gray'}>{status.toLowerCase()}</HrStatusPill></div>
      {progress.isLoading ? <p role="status">Loading cycle progress...</p> : progress.isError ? <PerformanceError error={progress.error} retry={() => progress.refetch()} /> : progress.data && <>
        ...
        <section><h4 className="mb-3 font-semibold">Assigned employees</h4>{!progress.data.reviewees.length ? <p className="text-sm text-text-secondary">No employee reviews assigned to this cycle yet.</p> : <div className="space-y-3">{progress.data.reviewees.map(employee => <div className="rounded-lg border border-border-default p-3" key={employee.revieweeId}><div className="flex items-center justify-between gap-2"><HrAvatar name={employee.revieweeName || 'Employee'} sub={employee.revieweeCode || employee.revieweeId} /><span className="whitespace-nowrap text-sm font-semibold">{employee.completedAssignments} / {employee.totalAssignments}</span></div><ul className="mt-3 space-y-1 border-t border-border-default pt-2 text-xs text-text-secondary">{employee.assignments.map(assignment => <li key={`${assignment.reviewerId}-${assignment.reviewerType}`} className="flex justify-between gap-2"><span>{assignment.reviewerName || assignment.reviewerId} · {assignment.reviewerType.toLowerCase().replaceAll('_', ' ')}</span><span>{assignment.status.toLowerCase().replaceAll('_', ' ')}</span></li>)}</ul></div>)}</div>}</section>
      </>}
      {canInitiate && status !== 'CLOSED' && <section className="space-y-4 border-t border-border-default pt-5"><h4 className="font-semibold">Assign reviews</h4><p className="text-sm text-text-secondary">Assigned reviewers complete these in My Reviews. Existing assignments are kept; only new assignments are added.</p>
