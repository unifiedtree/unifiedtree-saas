...
const statusLabel = (value?: string) => (value || 'ACTIVE').replaceAll('_', ' ').toLowerCase()
...
export function AdminKpis() {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><h2 className="text-lg font-semibold text-text-primary">Company goals & KPIs</h2><p className="mt-1 text-sm text-text-secondary">Set measurable targets, assign an owner, and keep a record of progress.</p></div>
      {manage && <HrButton onClick={() => setEditing('new')}><Plus size={16} /> Create KPI</HrButton>}
    </div>
    ...
    {query.isError ? <PerformanceError error={query.error} retry={() => query.refetch()} /> : <TableCard footer={<HrPagination page={page} pageSize={size} totalElements={query.data?.total ?? 0} totalPages={totalPages} onPageChange={setPage} onPageSizeChange={setSize} />}>
      <DataTable<EmployeeKpiRow> data={query.data?.items ?? []} keyField="id" loading={query.isLoading} emptyMessage={search || status ? 'No KPIs match these filters.' : 'No company KPIs yet. Create a target to start tracking progress.'} columns={[
        ...
        { key: 'owner', header: 'Owner', render: row => <HrAvatar name={row.ownerName || 'Employee'} sub={row.ownerCode || row.ownerId} /> },
...
function KpiDetails({ initial, canWrite, canManage, onClose }: { initial: EmployeeKpiRow; canWrite: boolean; canManage: boolean; onClose: () => void }) {
  ...
  return <HrDrawer title="KPI progress & history" width="max-w-xl" onClose={() => { if (!busy) onClose() }}>
    <div className="space-y-6">
      <div><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#0F6E56]"><Target size={14} className="mr-1 inline" /> {kpi.category || 'Company goal'}</p><h3 className="text-xl font-semibold">{kpi.title}</h3><p className="mt-2 text-sm text-text-secondary">{kpi.ownerName || kpi.ownerId} · {kpi.ownerCode} · {dateLabel(kpi.dueDate)}</p>{kpi.description && <p className="mt-3 text-sm">{kpi.description}</p>}</div>
