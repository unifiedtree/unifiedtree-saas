...
function BuildBatchDrawer({ initialCompany, onClose, onBuilt }: { initialCompany: string; onClose: () => void; onBuilt: (id: string) => void }) {
  ...
  return <HrDrawer title="Build reimbursement batch" onClose={() => { if (!build.isPending) onClose() }} footer={<><HrButton variant="ghost" disabled={build.isPending} onClick={onClose}>Cancel</HrButton><HrButton disabled={build.isPending || companies.isLoading || companies.isError} onClick={submit}>{build.isPending ? 'Building...' : 'Build draft batch'}</HrButton></>}><div className="space-y-5"><p className="text-sm text-text-secondary">Includes approved claims through the cutoff date that are not already reserved in another batch. An existing draft for the same company and cutoff is refreshed.</p>{companies.isError ? <ErrorMessage error={companies.error} retry={() => companies.refetch()} /> : <label className="block text-sm font-medium">Company<select aria-label="Company" className="ut-select mt-1" value={companyId} onChange={event => setCompany(event.target.value)}><option value="">Choose company</option>{companies.data?.map(company => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label>}<label className="block text-sm font-medium">Approval cutoff date<DateField className="ut-input mt-1" value={cutoffDate} onChange={event => setCutoff(event.target.value)} /></label><label className="block text-sm font-medium">Claim currency<input className="ut-input mt-1 uppercase" maxLength={3} value={currency} onChange={event => setCurrency(event.target.value.toUpperCase())} placeholder="INR" /><span className="mt-1 block text-xs font-normal text-text-secondary">Only approved claims in this currency are included.</span></label><label className="block text-sm font-medium">Batch notes<textarea className="ut-input mt-1" rows={3} value={notes} onChange={event => setNotes(event.target.value)} placeholder="Optional payment run context" /></label>{error && <p role="alert" className="text-sm text-red-700">{error}</p>}{build.isError && <ErrorMessage error={build.error} />}</div></HrDrawer>
}
