...
import { Download, Send, XCircle, Trash2, ChevronDown, ChevronUp } from 'lucide-react'
import { clsx } from 'clsx'
import { toast } from 'sonner'
import { Can, P } from '@unifiedtree/sdk'
import { HrStatusPill, HrButton, type PillTone } from '@/shared/components/hr'
...
export const GeneratedLetterDetail: React.FC = () => {
  ...
  const back = <HrButton variant="ghost" onClick={() => navigate('/hrms/letters/generated')}>← All letters</HrButton>
  if (isLoading || error || !letter) {
    return (
      <ModulePage crumb="Letters" title="Letter" actions={back}>
        {isLoading ? <State kind="loading" height={260} />
          : error ? <State kind="error" title="Couldn’t load the letter" description={(error as Error).message} onRetry={() => refetch()} />
            : <State kind="empty" icon="fileText" title="Letter not found" description="It may have been deleted." />}
      </ModulePage>
    )
  }
          ...
          <div className="ut-card p-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
              <div>
                ...
                <p className="text-sm text-text-primary">{stamp(letter.createdAt)}</p>
              </div>
            ...
            <div className="ut-card overflow-hidden">
              ...
              >
                <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider">
                  Merge values used ({contextEntries.length})
                </span>
          ...
          <div className="ut-card p-5 space-y-3">
            ...
            {letter.sentAt ? (
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-text-secondary">{stamp(letter.sentAt)}</p>
                {letter.sentToEmail && (
            ...
            {letter.voidedAt && (
              <div className="space-y-0.5 pt-2 border-t border-border-default">
                ...
                <p className="text-xs text-text-secondary">{stamp(letter.voidedAt)}</p>
                {letter.voidedReason && (
