...
// person may read it, and does the exports: the PDF snapshot (made on the
// server, sections following the reader's permissions), an Excel workbook, the
// departments CSV and PNGs of the two charts. Every file is recorded in the
// workspace's export log.
...
import { WorkforceAnalytics as WorkforceAnalyticsDesign, type ExportKind, type WaData, type WaDept, type WaGender, type WaMonth } from '@/design/dc/WorkforceAnalytics'
import { apiBlob } from '@/core/api/client'
import { csvBlob, xlsxBlob, svgToPng, saveAndRecord, saveServerFile, type Cell } from '@/shared/export/fileExport'
import { stackedBarsSvg, lineSvg } from '@/shared/export/charts'
...
export function WorkforceAnalytics() {
  ...
  const onDept = (r: WaDept) => { if (canDirectory) navigate(`/hrms/employees?co=${co.company}&departmentId=${r.id || 'none'}`) }

  // ── exports ──
  const base = `workforce-analytics-${slug(co.companyName)}-${range.value}`
  const meta = (fmt: 'CSV' | 'XLSX' | 'PNG', rows?: number) => ({ report: 'workforce-analytics' as const, fmt, companyId: co.company, filters: { period: range.label, from: range.from, to: range.to, asOf }, rows })
  ...
  const onExport = async (k: ExportKind): Promise<string> => {
    if (k === 'csv') {
      ...
      saveAndRecord(file, csvBlob(deptRows()), meta('CSV', depts.length))
    ...
    if (k === 'xlsx') {
      ...
      saveAndRecord(file, xlsxBlob(sheets), meta('XLSX', depts.length + months.length))
      return `${file} downloaded`
    }
    ...
    // PDF: made on the server with the same numbers; each section only with its own report permission.
    const blob = await apiBlob(`/v1/reports/workforce-analytics/export.pdf?${new URLSearchParams({ companyId: co.company, from: range.from, to: range.to })}`)
    const file = `${base}.pdf`
