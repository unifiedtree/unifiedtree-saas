...
async function branches({ added, changed }: Diff, env: SyncEnv) {
  for (const r of added) {
    ...
    const b = await apiJson<{ id: string }>('/v1/hrms/branches', json('POST', { companyId: r.co, name: String(r.name).trim(), code: blank(r.code), city: blank(r.city), state: blank(r.state), isHeadquarters: hq, branchType: BRANCH_KIND_CODE[r.kind] || 'BRANCH' }))
