                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      selected === globalIdx ? 'bg-[#ECFDF5] text-[#047857]' : 'text-text-secondary hover:bg-[#ECFDF5]'
                    )}
                  >
                    <span className={selected === globalIdx ? 'text-[#047857]' : 'text-text-tertiary'}>{cmd.icon}</span>
                    <span className="flex-1 text-left">{cmd.label}</span>
                    {cmd.description && <span className="text-xs text-text-tertiary">{cmd.description}</span>}
                    {selected === globalIdx && <ChevronRight size={14} className="text-[#047857]" />}
                  </button>
                )
              })}
            </div>
