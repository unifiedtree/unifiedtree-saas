...
export class PayrollRunPage extends DCLogic {
  ...
  renderVals() {
    ...
    const M: Record<string, any> = {
      process: { title: 'Process payroll?', desc: `Pay for ${count != null ? who(count) : 'everyone with a salary structure'} will be calculated for ${run?.period || ''} from attendance and loan recoveries.`, confirm: 'Process payroll', go: () => act(A.process, true), skip: true },
      reprocess: { title: 'Re-process payroll?', desc: 'All payslips will be recalculated with the latest attendance, salary structures and recoveries. The current figures will be replaced.', confirm: 'Re-process', go: () => act(A.process, true), skip: true },
      lock: { title: 'Lock this payroll run? Payslips become final.', desc: 'Employees can see their payslips and you can prepare the bank file. To change anything after this, you’ll need to reopen the run with a reason.', confirm: 'Lock run', go: () => act(A.lock) },
      reopen: { title: 'Reopen payroll?', desc: 'Payslips go back to draft so you can correct them. You’ll need to lock the run again before paying.', confirm: 'Reopen payroll', go: () => act(() => A.reopen && A.reopen(s.reason.trim())), reopen: true },
