...
export function PlatformShell() {
  ...
  // The page name is the label of the visible nav link with the longest path
  // matching this route (the same links and rules the menu renders).
  const pageLabel = React.useMemo(() => {
    const links: { label: string; path: string }[] = []
    for (const item of [...NAV_ITEMS, ...PLATFORM_ITEMS, ...SETTINGS_NAV]) {
      if (item.path && isVisible(item)) links.push({ label: item.label, path: item.path })
    }
    for (const group of MODULE_ITEMS) {
      for (const child of group.children ?? []) {
        if (child.path && isVisible(child, group.key)) links.push({ label: child.label, path: child.path })
      }
    }
    const best = links
      .filter(l => matchPath(location.pathname, l.path.split('?')[0]))
