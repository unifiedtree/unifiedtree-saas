...
export const PATCHES = {
  ...
  '444d22cf': [
    // Branch types (V143.22): the task's list — Head office, Branch, Plant, Warehouse, Office, Store, Other.
    ["const BRANCH_KIND_TONE={'Head office':'brand',", "const BRANCH_KIND_TONE={'Head office':'brand','Branch':'cyan','Store':'violet','Other':'slate',"],
    // The backend's computation types: FORMULA and STATUTORY. A FIXED component's amount is its own
    // (payroll applies it); without one, amounts are set on each salary structure. Lines payroll works
    // out itself (PLI, advance recovery, LWF) say where they come from.
    ["const METHODS={pct_ctc:{l:'% of CTC',d:c=>c.val+'% of CTC'},", "const METHODS={formula:{l:'Formula',d:c=>c.note||'Set on each salary structure'},statutory:{l:'Statutory',d:c=>c.note||'From Statutory Settings'},pct_ctc:{l:'% of CTC',d:c=>c.val+'% of CTC'},"],
    ["fixed:{l:'Fixed amount',d:c=>fmtINR(c.val)+' / month'}", "fixed:{l:'Fixed amount',d:c=>c.note||(c.val!=null?fmtINR(c.val)+' / month':'Set on each salary structure')}"],
  ],
  ...
  d6399539: [
    ...
    ["value={total} sub={'Deployed across '+new Set(A.filter(a=>a.status==='Active').flatMap(a=>a.sites)).size+' sites'}", "value={total} sub={'Deployed across '+pl(new Set(A.filter(a=>a.status==='Active').flatMap(a=>a.sites||[])).size,'site','sites')}"],
    ["t={expiring.length?'red':'green'} label=\"Licences expiring\" value={expiring.length} sub={expiring.length?expiring[0].name+' · '+daysTo(expiring[0].licence)+' days':'All valid for 30+ days'}", "t={expiring.length?'red':'green'} label=\"Licences expiring\" value={expiring.length} sub={expiring.length?expiring[0].name+' · '+(daysTo(expiring[0].licence)<0?'expired':daysTo(expiring[0].licence)+' days'):A.some(a=>a.status==='Active'&&a.licence)?'All valid for 30+ days':'No licence dates added yet'}"],
    ...
    ["<dd>{a.sites.map(s=>M.branch[s].name).join(', ')}</dd>", "<dd>{a.sites&&a.sites.length?a.sites.map(s=>M.branch[s].name).join(', '):'—'}</dd>"],
    ["<span>Licence {dd<0?'expired':'valid till'} {fmtDate(a.licence)}</span>", "<span>{a.licence?'Licence '+(a.licenceNo?a.licenceNo+' ':'')+(dd<0?'expired':'valid till')+' '+fmtDate(a.licence):'No licence date added'}</span>"],
    ['<span className="muted">Since {a.since}</span>', "<span className=\"muted\">{a.since?'Since '+a.since:''}</span>"],
    ["body={end.workers+' contract workers will be released from '+end.sites.map(s=>M.branch[s].name).join(', ')+'. Their attendance stops syncing today.'}", "body={'The agency is marked inactive. '+pl(end.workers||0,'contract worker stays','contract workers stay')+' linked on their employee records, and you can reactivate the agency later.'}"],
    ["validate:x=>x&&x.length?null:'Pick at least one site'}", "validate:x=>!db.branches.length||x&&x.length?null:'Pick at least one site'}"],
    ["{k:'workers',label:'Active workers',type:'number',min:0,suffix:'people'}", "{k:'workers',label:'Active workers',type:'number',min:0,suffix:'people',disabled:true,hint:'Counted from the contract workers linked to this agency on their employee record'}"],
    ["{k:'licence',label:'Licence valid till',type:'date',req:true,hint:'CLRA licence — we remind you 30 days before expiry'}", "{k:'licenceNo',label:'Licence number',placeholder:'e.g. CLRA/KA/2021/0077'},{k:'licence',label:'Licence valid till',type:'date',hint:'CLRA licence — flagged on this page and the overview 30 days before it expires'}"],
    ["const rec=Object.assign({t:'teal',status:'Active',since:'2026'},a||{},v,{id:isEdit?a.id:'A'+Date.now(),workers:+v.workers||0});", "const rec=Object.assign({t:'teal',status:'Active',since:''},a||{},v,{id:isEdit?a.id:'A'+Date.now(),workers:isEdit?a.workers:0});"],
    // Renewing a licence with no date on record opens the form to enter one.
    ["const renew=a=>{const dt=asDate(a.licence);", "const renew=a=>{if(!a.licence){setForm(a);return}const dt=asDate(a.licence);"],
    // Deployment sites: active branches of the agency's company.
    ["options:db.branches.map(b=>({v:b.id,l:b.city}))", "options:db.branches.filter(b=>b.status==='Active'&&b.co===((a&&a.co)||(db.companies[0]||{}).id)).map(b=>({v:b.id,l:b.city||b.name}))"],
    // Employee form: a contract worker's staffing agency (the link the agency's worker count is counted from).
  ...
  aa085547: [
    ...
    // Companies: head office comes from the branch marked HQ; TAN, date of incorporation ("since") and a description are stored (V143.22).
    ['<small>{c.legal} · {c.industry}</small>', "<small>{[c.legal,c.industry].filter(Boolean).join(' · ')||'Legal name not set'}</small>{c.desc&&<small>{c.desc}</small>}"],
    ...
    ["{k:'TAN',label:'TAN',upper:true,placeholder:'AAAA00000A'}", "{k:'TAN',label:'TAN',upper:true,placeholder:'AAAA00000A',validate:x=>!x||/^[A-Z]{4}[0-9]{5}[A-Z]$/.test(x)?null:'Format: AAAA00000A'}"],
    ["const rec=Object.assign({t:'teal',status:'Active',since:'2026'},c||{},{", "const rec=Object.assign({t:'teal',status:'Active',since:''},c||{},{"],
    ["industry:v.industry,hq:v.hq||'—',ids:{", "industry:v.industry,hq:v.hq||'—',inc:v.inc||'',desc:v.desc||'',since:v.inc?String(v.inc).slice(0,4):'',ids:{"],
    // Branches: code is shown, ids are keys; branch types are stored (V143.22); inactive branches show with the status filter.
    ...
    ["<Dropdown label=\"Company\" all=\"All companies\" value={co} options={db.companies.map(c=>({v:c.id,l:c.name}))} onChange={setCo} icon=\"landmark\"/>", "<Dropdown label=\"Company\" all=\"All companies\" value={co} options={db.companies.map(c=>({v:c.id,l:c.name}))} onChange={setCo} icon=\"landmark\"/><Dropdown label=\"Status\" all=\"Active branches\" value={route.archived?'all':''} options={[{v:'all',l:'Include inactive'}]} onChange={x=>act.showArchivedBranches(!!x,co)} icon=\"archive\"/>"],
    // Departments: head by id, code shown; a department moves under another (not under its own sub-teams) and records its branches (V143.22).
    ...
    ["hint:'Sub-teams roll their headcount up into the parent'}", "disabled:isEdit&&db.depts.some(y=>y.parent===x.id),hint:isEdit&&db.depts.some(y=>y.parent===x.id)?'It has sub-teams of its own, so it stays top-level. Move its sub-teams out first.':'Sub-teams roll their headcount up into the parent'}"],
    ["function DeptForm({x,init,onClose}){\nconst {db,update,toast}=useApp();", "function DeptForm({x,init,onClose}){\nconst {db,update,toast,act}=useApp();"],
    ["{k:'icon',type:'custom',render:", "{k:'branches',label:'Branches',type:'multi',span2:true,options:v=>{const co=v.parent?(db.depts.find(y=>y.id===v.parent)||{}).co:(x?x.co:act.defaultCo);return db.branches.filter(b=>b.co===co&&(b.status==='Active'||(v.branches||[]).includes(b.id))).map(b=>({v:b.id,l:b.name}))},hint:'Where this department works. Leave all off if it works at every branch.'},\n{k:'icon',type:'custom',render:"],
    // Designations: codes are stored; the grade is linked by id (V143.22), and legacy text that matches no grade shows as a chip.
    ...
    ["{k:'name',label:'Job title',req:true,span2:true,placeholder:'e.g. Data Analyst'}", "{k:'name',label:'Job title',req:true,span2:true,placeholder:'e.g. Data Analyst'},{k:'code',label:'Code',upper:true,placeholder:'e.g. SSE',hint:'Optional short code, unique in the company',validate:c=>c&&db.desigs.some(y=>(y.code||'')===c&&(!isEdit||y.id!==x.id))?'Code already in use':null}"],
    // Grades: ordered by level; pay bands are stored (V143.22) and hidden from people without hrms.grade.band.read; grades can be switched off (as in Org Setup).
    ...
    ["<td><span className=\"t1 num\">{fmtL(g.min)} – {fmtL(g.max)}</span><span className=\"t2\">{ov?'Overlaps '+prev.id+' by '+fmtL(ov):'Midpoint '+fmtL((g.min+g.max)/2)}</span></td>", "<td>{g.min==null?<><span className=\"no\">—</span><span className=\"t2\">{g.hidden?'Pay band hidden':'Band not set yet'}</span></>:<><span className=\"t1 num\">{fmtL(g.min)} – {fmtL(g.max)}</span><span className=\"t2\">{ov?'Overlaps '+prev.id+' by '+fmtL(ov):'Midpoint '+fmtL((g.min+g.max)/2)}</span></>}</td>"],
    ...
    ["{k:'min',label:'Minimum',type:'number',req:true,prefix:'₹',min:0,step:50000},{k:'max',label:'Maximum',type:'number',req:true,prefix:'₹',min:0,step:50000,validate:(x,v)=>+x>+v.min?null:'Must be more than the minimum'}", "{k:'min',label:'Minimum',type:'number',prefix:'₹',min:0,step:50000,disabled:!act.canBands,hint:act.canBands?'Leave both empty if this grade has no band':'You don’t have access to pay bands',validate:(x,v)=>(x===''||x==null)===(v.max===''||v.max==null)?null:'Enter both amounts, or neither'},{k:'max',label:'Maximum',type:'number',prefix:'₹',min:0,step:50000,disabled:!act.canBands,validate:(x,v)=>x===''||x==null||+x>+v.min?null:'Must be more than the minimum'}"],
    ["const rec=Object.assign({t:'teal',status:'Active'},g||{},v,{min:+v.min,max:+v.max});", "const rec=Object.assign({t:'teal',status:'Active'},g||{},v,act.canBands?{min:v.min===''||v.min==null?null:+v.min,max:v.max===''||v.max==null?null:+v.max}:{min:g?g.min:null,max:g?g.max:null});"],
    ['sub="Bands guide offers and revisions — payroll flags salaries outside the band."', 'sub="Bands guide offers and revisions — Salary Structure warns when someone’s pay falls outside the band."'],
    ["function GradeForm({g,onClose}){\nconst {db,update,toast}=useApp();", "function GradeForm({g,onClose}){\nconst {db,update,toast,act}=useApp();"],
  ],

  // ── Shift Rules · Leave Rules · Policy Documents ─────────────────────────
  '68265841': [
    // Shifts: duplicating opens a prefilled "Add shift" (there's no inactive state to park a copy in); the copy gets no code (codes are unique).
    ["const dup=s=>{let id=s.id+'2';while(db.shifts.some(x=>x.id===id))id+='2';update('shifts',L=>L.concat([Object.assign({},s,{id,name:s.name+' (copy)',status:'Inactive'})]));toast('Duplicated '+s.name+' — edit and activate the copy')};", "const dup=s=>setForm({copy:Object.assign({},s,{name:s.name+' (copy)',code:''})});"],
    ...
    // Shift code (V143.23): optional, unique among active shifts, saved with the shift.
    ["{k:'id',label:'Code',req:true,upper:true,placeholder:'e.g. GEN',validate:x=>!isEdit&&db.shifts.some(y=>y.id===x)?'Code already in use':null}", "{k:'code',label:'Code',upper:true,placeholder:'e.g. GEN',hint:'Optional. A short code for rosters and reports',validate:x=>x&&!/^[A-Z0-9_-]{1,20}$/.test(x)?'Up to 20 letters, digits, - or _':x&&db.shifts.some(y=>y.status==='Active'&&y.code===x&&(!isEdit||y.id!==s.id))?'Code already in use':null}"],
    ["const rec=Object.assign({icon:'clock',t:'teal'},s||{},{id:v.id,name:v.name,kind:v.kind,", "const rec=Object.assign({icon:'clock',t:'teal'},s||{},{id:v.id,code:v.code||'',name:v.name,kind:v.kind,"],
    ...
    // Core hours (V143.23): a flexible shift is late after core start.
    ["{k:'coreS',label:'Core hours from',type:'time',when:v=>v.kind==='Flexible'},{k:'coreE',label:'Core hours until',type:'time',when:v=>v.kind==='Flexible'}", "{k:'coreS',label:'Core hours from',type:'time',req:true,when:v=>v.kind==='Flexible',hint:'Check-ins after this are marked Late'},{k:'coreE',label:'Core hours until',type:'time',req:true,when:v=>v.kind==='Flexible',validate:(x,v)=>toMin(x)<=toMin(v.coreS)?'Must end after core hours start':toMin(v.startS)<toMin(v.endS)&&(toMin(v.coreS)<toMin(v.startS)||toMin(x)>toMin(v.endS))?'Keep core hours inside the shift':null}"],
    // Weekly offs per shift (V143.23): used for people with no weekly offs of their own.
    ["{k:'offs',label:'Weekly off',type:'multi',span2:true,options:WEEK}", "{k:'offs',label:'Weekly off',type:'multi',span2:true,options:WEEK,hint:'Used for people on this shift who have no weekly offs on their own record. Leave empty for Saturday and Sunday.',validate:x=>x&&x.length>=7?'Leave at least one working day':null}"],
    ...
    ["'Everyone must be online between '+(v.coreS||'—')+' and '+(v.coreE||'—')+'; the rest of the window is flexible.'", "'Everyone is expected between '+(v.coreS||'—')+' and '+(v.coreE||'—')+'; a check-in after '+(v.coreS||'—')+' is marked Late. The rest of the window is flexible.'"],
    ...
    // Leave: codes shown; quota must be > 0; applies to everyone.
    ...
    ["<span className=\"t2\">{l.encash?'Balance encashable at exit':'Not encashable'}</span>", "<span className=\"t2\">{l.encash?'Encashable'+(l.encashMax?' · up to '+l.encashMax+' days a year':''):'Not encashable'}</span>"],
    ...
    // Accrual (V143.23): credited by the daily accrual job.
    ["{k:'accrual',label:'Credited',type:'seg',options:['Monthly','Quarterly','Upfront']}", "{k:'accrual',label:'Credited',type:'seg',options:['Monthly','Quarterly','Upfront'],hint:'Monthly: a twelfth of the quota on the 1st of each month. Quarterly: a quarter on 1 Jan, Apr, Jul and Oct. Upfront: the whole quota at the start of the year. Monthly and quarterly credits start from the month someone joins.'}"],
    // Encashment (V143.23): requests with HR approval, paid by the next payroll run.
    ["{k:'encash',label:'Encashable',type:'toggle',hint:'Unused balance is paid out at exit',when:v=>v.carryOn}", "{k:'encash',label:'Encashable',type:'toggle',hint:'Employees can ask to cash in unused days. Once HR approves, the days come off the balance and are paid in the next payroll run at one day’s Basic each.',when:v=>v.carryOn},{k:'encashMax',label:'Most days encashed a year',type:'number',min:1,max:365,suffix:'days max',when:v=>v.carryOn&&v.encash,hint:'Per person. Leave empty for no yearly limit (the balance is still the limit).',validate:x=>x===''||x==null||(+x>=1&&+x<=365)?null:'Between 1 and 365'}"],
    ["encash:!!(v.carryOn&&v.encash),", "encash:!!(v.carryOn&&v.encash),encashMax:v.carryOn&&v.encash&&v.encashMax!==''&&v.encashMax!=null?+v.encashMax:null,"],
    ["const got=v.accrual==='Monthly'?q/12*3:v.accrual==='Quarterly'?q/4:q;", "const nm=+NEXT_MONTH.slice(5,7);const got=v.accrual==='Monthly'?q/12*(13-nm):v.accrual==='Quarterly'?q/4*(4-Math.floor((nm-1)/3)):q;"],
    ["{v.accrual==='Upfront'?'Full quota credited on joining':'Pro-rated for Oct–Dec'}", "{v.accrual==='Upfront'?'Full quota credited on joining':v.accrual==='Monthly'?'A twelfth credited on the 1st of each month':'A quarter credited at the start of each quarter'}"],
    ['sub="Balances recalculate for everyone it applies to on the next accrual run."', 'sub="Monthly and quarterly credits follow these rules from the next accrual run; days already credited stay."'],
    ...
    // Policies: real categories; a new version is a new draft.
    ...
    // Remind (V143.23): everyone who hasn't acknowledged, not anyone reminded in the last 24 hours.
    ["const {db,d,update,toast}=useApp();const [tab,setTab]=React.useState('Active');", "const {db,d,update,toast,act:ax}=useApp();const [del,setDel]=React.useState(null);const [tab,setTab]=React.useState('Active');"],
    ["{icon:'bell',label:'Remind '+(hc-p.ack)+' people',on:()=>toast('Reminder sent to '+(hc-p.ack)+' people')}", "{icon:'bell',label:'Remind '+(hc-p.ack)+' people',on:()=>ax.remindPolicy(p)}"],
    // Discard draft deletes it for good (V143.23), after a confirmation; published policies archive.
    ["{icon:'trash',label:'Discard draft',danger:true,on:()=>{update('policies',L=>L.filter(x=>x.id!==p.id));toast('Draft discarded')}}", "{icon:'trash',label:'Discard draft',danger:true,on:()=>setDel(p)}"],
    ["{form&&<PolicyForm p={form.id?form:null} onClose={()=>setForm(null)} onDone={s=>setTab(s)}/>}", "{form&&<PolicyForm p={form.id?form:null} onClose={()=>setForm(null)} onDone={s=>setTab(s)}/>}{del&&<Modal title={'Discard '+del.title+'?'} body=\"The draft is deleted for good. It was never published, so nobody has seen it.\" icon=\"trash\" t=\"red\" danger cta=\"Discard draft\" onClose={()=>setDel(null)} onOk={()=>{const x=del;setDel(null);update('policies',L=>L.filter(y=>y.id!==x.id));toast('Draft discarded')}}/>}"],
    ...
    ['delete rec.notify;delete rec.content;', ''],
    ["+(v.notify?' and emailed to '+d.headcount+' people':'')", "+(v.notify&&(!isEdit||p.status==='Draft')?' · emailing '+pl(d.headcount,'person','people')+' now':'')"],
  ...
  '99fffcea': [
    ...
    // "On payslip" works as designed (V143.11); a hidden component is added into one "Other" line.
    ['On payslip<Tip>Hidden components still count toward CTC — they just don’t print as a line on the payslip.</Tip>', 'On payslip<Tip>Hidden components still count toward pay — on the payslip and its PDF they’re added into one “Other earnings” or “Other deductions” line.</Tip>'],
    ["toast(c.name+(x?' now prints':' hidden')+' on payslips')", "toast(c.name+(x?' now prints as its own line on payslips':' now prints inside “Other” on payslips'))"],
    // Components can be switched off (built-in ones stay on: payroll depends on them), after a
    // warning, because pay changes from the next run. Unused ones can be deleted (as on the old page).
    ...
    ["const {db,update,toast,go}=useApp();const [cat,setCat]=React.useState('');", "const {db,update,toast,go}=useApp();const [del,setDel]=React.useState(null);const [off,setOff]=React.useState(null);const [cat,setCat]=React.useState('');"],
    ["toast(c.name+(st==='Active'?' activated':' deactivated — skipped from the next run'))", "toast(c.name+(st==='Active'?' switched on — paid from the next run':' switched off — skipped from the next run'))"],
    ...
    // A fixed component's own monthly amount (V143.11). Money: the hint says exactly who it's paid to or taken from.
    ["{k:'valAmt',label:'Amount per month',type:'number',req:true,min:0,prefix:'₹',when:isAmt}", "{k:'valAmt',label:'Amount per month',type:'number',min:0,prefix:'₹',when:isAmt,disabled:!!(c&&c.managed),hint:c&&c.managed?'Worked out by payroll — there’s no amount to set':'Optional. An earning is paid every month to everyone whose salary structure doesn’t list it (less for unpaid days). A deduction is taken in full from everyone unless their salary structure sets its own amount. Leave empty to set amounts on each salary structure.'}"],
    ["{v:'Partly',l:'Partly exempt'}", "{v:'Partly',l:'Partly exempt',disabled:true}"],
    ["{k:'payslip',label:'Show on payslip',type:'toggle',hint:'Hidden components still count toward CTC'},{k:'active',label:'Active',type:'toggle',hint:'Inactive components are skipped in the next payroll run'}", "{k:'payslip',label:'Show on payslip',type:'toggle',hint:'Off: it still counts toward pay, and prints inside one “Other earnings” or “Other deductions” line'},{k:'active',label:'Active',type:'toggle',disabled:!!(c&&c.system),hint:c&&c.system?'Built-in components are always on':'Off: payroll stops paying or deducting it from the next run. Runs already processed don’t change.'}"],
