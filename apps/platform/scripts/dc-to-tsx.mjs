...
const helmetCss = [...xdc.matchAll(/<helmet>([\s\S]*?)<\/helmet>/g)]
...
if (pseudo.size || helmetCss.length) imports.push(`import './${name}.view.css'`)
...
if (pseudo.size || helmetCss.length) writeFileSync(join(outDir, `${name}.view.css`), [...helmetCss, ...[...pseudo.values()].map((p) => p.rule)].join('\n') + '\n')
