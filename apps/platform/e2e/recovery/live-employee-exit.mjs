...
try {
  ...
  await page.getByRole('button', { name: 'Mark Exited', exact: true }).click()
  const exitModal = page.locator('div.ut-card').filter({ has: page.getByRole('heading', { name: 'Mark Employee as Exited', exact: true }) })
  ...
  assert.equal((await response).status(), 200)
  await expect(page.getByRole('heading', { name: 'Mark Employee as Exited', exact: true })).toHaveCount(0)
  saved = await read()
