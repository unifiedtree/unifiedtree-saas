-- V143.30: restore the Professional Tax slabs.
--
-- payroll.pt_slabs is shared reference data (no tenant_id) seeded by V047, but
-- production had 0 rows (lost in the fresh-launch wipe), so PT was Rs 0 on every
-- payslip even for companies with PT switched on. Same rows as V047; idempotent.
