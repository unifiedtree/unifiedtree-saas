...
public class PayrollReportService {
    ...
    /** Optional: the workspace's own letterhead (logo + company name). */
    ...
    public byte[] salaryRegisterPdf(UUID tenantId, UUID runId) {
        ...
        // White label: the register opens with the workspace's own letterhead.
        if (letterhead != null) html = letterhead.applyToDocument(html, tenantId, (String) run.get("company_name"));
    ...
    private static String renderSalaryRegisterHtml(Map<String, Object> run, List<SalaryRegisterRow> rows) {
        ...
        b.append("<p class='sub' style='margin-top:16pt'>Generated on ")
         .append(java.time.LocalDate.now(java.time.ZoneId.of("Asia/Kolkata"))).append("</p>");
        b.append("</body></html>");
        return b.toString();
    }
