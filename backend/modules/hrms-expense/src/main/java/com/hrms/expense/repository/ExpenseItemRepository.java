...
public interface ExpenseItemRepository extends JpaRepository<ExpenseItem, UUID> {
    ...
    /** Per claim: [claimId, line count, lines with a receipt]. One query for a whole page of claims. */
    @org.springframework.data.jpa.repository.Query(
            "select i.claimId, count(i), count(i.receiptUrl) from ExpenseItem i where i.claimId in :claimIds group by i.claimId")
