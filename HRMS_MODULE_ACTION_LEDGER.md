...
|---|---|---|---|
| Auth / login | Two `POST /v1/canonical-auth/refresh → 422 REFRESH_MISSING` on every cold load of `/login`. | Trace: fired from `packages/sdk/src/auth/authStore.ts` `tryRestoreSession()` before sign-in; 422 = no refresh cookie = "not signed in" (documented contract); ×2 = React StrictMode double effect in dev. No 401s from app requests after login. | **Explained — by design, not a defect.** Evidence scripts reset error counters after login. Side note: no refresh cookie is set on http://demo.localhost (cookie list empty after login) — local runtime cannot exercise cookie-based restore; production https path unverified here. |
...
## 5. Backend increments awaiting live verification (jar built 2026-09-24, restart pending until batch-1 live tests finish)

| Item | Change | Unit evidence | Live evidence (to run after restart) |
|---|---|---|---|
| Late-by columns (C1 follow-up, plan item 5) | `AttendanceService.getShiftWindowsForEmployees` (same date-aware "in force on the date" predicate as the early-checkout lookup) feeds `StaffStatusResponse` + `shiftName`, `expectedCheckInAt`, `graceMinutes`, `lateByMinutes` (LATE rows only). Early-checkout now derives from the same single query. | `StaffStatusLateByTest` 2/2 | extend `live-late-drilldown.mjs`: row shows shift, expected 09:00, late by 47m (needs Attendance.tsx columns — after the batch-1 face-punch verifier releases the file) |
| Workforce Directory CSV export (plan item 17, P0) | `GET /v1/hrms/employees/export.csv` — same filters + `hrms.employee.read` as the JSON directory, paged through the directory service (RLS + list masking unchanged), names for department/designation/branch/manager, **no** salary/bank/identity/DOB columns, RFC-4180 + formula-injection guard, BOM, 10,000-row cap with `X-Export-Truncated`/`X-Export-Total`. Employees.tsx "Export CSV" button exports exactly the current filters and warns above the cap. | `EmployeeDirectoryCsvTest` 3/3 | `live-directory-export.mjs` (API contract, formula guard, status filter, 403 for employee, browser download) |
| Candidate → employee conversion (plan item 11, AT-3) | V140 adds `hiring_mgmt.candidates.converted_employee_id/converted_at` (+ unique index). `POST /v1/hiring/candidates/{id}/convert` (needs `hrms.hiring.candidate.write` AND `hrms.employee.write`): one transaction — lock HIRED candidate (409 if already converted, 422 if not HIRED), create the employee via the ordinary `WorkforceEmployeeService.create` (seat quota, validation, code generation, designation from the offer role), link the candidate. Carries name/email/phone, company/department/employment type, accepted-offer role, joining date and CTC. Pipeline shows "Convert to employee" for HIRED rows and "View employee" once converted. | `CandidateConversionServiceTest` 2/2; `HiringOfferWorkflowTest` 6/6, `OfferDocumentTest`, `OfferDeliveryTest` still pass | `live-candidate-conversion.mjs` (requisition → HIRED → accepted offer → UI convert → facts carried → link → 409 on repeat → 422 non-HIRED → 403 employee → UI after reload) |

Follow-up recorded: the onboarding wizard (`/hrms/onboarding/instances/new`) creates its own employee, so a converted candidate must NOT be sent there (duplicate). Starting an onboarding checklist for an existing employee needs a picker on the instances page — plan item for batch 2.
