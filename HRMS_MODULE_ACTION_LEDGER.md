...
## 3. Unresolved business inputs (asked, not answered)

1. Overtime compensation rule (paid components / divisor / multiplier, comp-off, or approval-only; caps, rounding, cutoff, reversal).
2. Required external providers (email/SMS/bank/storage) and where their test configuration lives.
3. Whether a delivered inspector OTP is still required (signed expiring links are implemented).
4. Confirmation to split `/hrms/exit` from `/hrms/fnf` (planned unless vetoed).
