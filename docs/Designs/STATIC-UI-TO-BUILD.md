...
- **Archived companies can be restored** (26 Sep, after wave 3): Companies & Branches → Inactive lists archived companies with Restore, like archived branches (`POST /v1/hrms/companies/{id}/restore`, `GET /v1/hrms/companies?includeArchived=true`; existing company permissions; no migration). Archiving is refused for the last active company and for a company with active employees, on the server and in the page. Test: `live-company-restore.mjs`.
