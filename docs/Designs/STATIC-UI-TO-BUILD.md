...
Checked live (API): `e2e/recovery/live-w2g.mjs` covers Profile, Security (two-factor, sessions, the workspace rule) and the Danger zone, with 403s for refused roles and database checks; it cleans up after itself. The notes below about Security's "Coming soon" sections and the danger-zone email links describe the page before w2g.
